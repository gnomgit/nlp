...
@Getter
@Setter
public class PersonFinderNER implements NER {

	private NameFinderME nameFinderEn;
	
	public PersonFinderNER () {
		initialize ();
	}

	public PersonFinderNER initialize () {
		try {
			TokenNameFinderModel lnfModel = new TokenNameFinderModel(PersonFinderNER.class.getClassLoader().getResourceAsStream(Constants.NER_EN_PERSON));
	...
	public Sentence annotate(Sentence sentence) {
		...
		for (Span span : spansEn) {
			sentence.getWords().get(span.getStart()).setNerTag(Constants.B + Constants.PERSON);
			sentence.getWords().get(span.getStart()).setNerProb(nameFinderEn.probs()[span.getStart()]);
			
			for (int i = span.getStart() + 1; i < span.getEnd(); i++) {
				sentence.getWords().get(i).setNerTag(Constants.I + Constants.PERSON);
